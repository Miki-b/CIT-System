...
export async function seedDemoData(
  ...
  await prisma.driver.createMany({
    data: [
      { name: "Grace Okoro", phone: "+1 (212) 555-0210", status: "AVAILABLE" },
      { name: "Liam Novak", phone: "+1 (212) 555-0233", status: "AVAILABLE" },
    ],
  });
  ...
  const pickup1001 = { lat: 40.7089, lng: -74.0089 };
  const dest1001 = { lat: 40.7033, lng: -74.017 };
  ...
  const pickup1002 = { lat: 40.758, lng: -73.9855 };
  const dest1002 = { lat: 40.7527, lng: -73.9772 };
  ...
  const pickup1003 = { lat: 40.6928, lng: -73.9903 };
  const dest1003 = { lat: 40.6446, lng: -73.7797 };
...
export async function ensureDemoData(prisma: PrismaClient) {
  seedPromise ??= seedDemoData(prisma).catch((error) => {
    seedPromise = null;
    throw error;
  });
  await seedPromise;
